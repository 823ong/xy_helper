  ...
  <div class="p-6">
    <n-card title="设置">
      <template #header-extra>
        <n-button @click="handleSaveConfig" :loading="loadingObj.saveLoading">
          保存
        </n-button>
      </template>
            ...
            <n-card title="检查更新">
              <n-space vertical>
                <n-space align="center">
                  ...
                  <n-text v-if="updateInfo" depth="3">
                ...
                <!-- 无更新提示 -->
                <n-alert v-if="!hasUpdate && checked" type="info">
                  已是最新版本
                </n-alert>
              </n-space>
            </n-card>
...
import { NButton, NCard, NForm, NFormItem, NInput, NRadioButton, NRadioGroup, NTabPane, NTabs, NSpace, NSelect, NPopconfirm, NModal, NText, NAlert, NTag, NProgress } from "naive-ui";
import { onMounted, reactive, ref } from "vue";
import { win } from "@renderer/win";
import { useSystemSettingsStore } from "@renderer/stores/systemSettings";
import { SystemSettings, SyncConfig, UpdateInfo } from "../../../../preload/types/api";
...
const loadSyncConfigs = async () => {
  const configs = await win.api.systemSettings.listSyncConfigs();
  syncConfigs.value = configs;
};
...
const handleInstallUpdate = async () => {
  const dialogResult = await win.api.electron.dialog.showMessageBox({
    type: 'info',
    title: '安装更新',
    message: '更新下载完成，应用将重启并安装更新。确定继续吗?',
    buttons: ['确定', '取消']
  });
